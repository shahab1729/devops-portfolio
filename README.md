# Shahab Shaikh | DevOps Portfolio

Welcome to my personal DevOps portfolio and interactive web application! I am an aspiring DevOps Engineer passionate about infrastructure automation, containerization, cloud systems management (AWS), and building reliable CI/CD deployment pipelines.

---

## 👤 About Me

- 🔗 **Live Portfolio**: [shahab1729.github.io/devops-portfolio](https://shahab1729.github.io/devops-portfolio/)
- 💻 **Role**: Aspiring DevOps Engineer
- 🎯 **Current Focus**: Linux • Bash • Networking • Git • Docker (hands-on so far, learning AWS & CI/CD next)
- 📍 **Location**: Remote / Open to Relocation
- 📧 **Email**: [shahabshaikh70@gmail.com](mailto:shahabshaikh70@gmail.com)
- 🌐 **GitHub**: [github.com/shahab1729](https://github.com/shahab1729)
- 🔗 **LinkedIn**: [linkedin.com/in/shahab-shaikh](https://www.linkedin.com/in/shahab-shaikh)

> *"Passionate aspiring DevOps engineer dedicated to mastering infrastructure automation, containerized application delivery, and cloud management. Focused on building practical hands-on labs, writing shell scripts, configuring cloud resources on AWS, and mastering modern CI/CD pipelines."*

---

## 🛠️ Technical Skills & Core Competencies

| Category | Skills & Tools |
| :--- | :--- |
| **Operating Systems & Scripting** | Linux (Ubuntu/Debian), Bash Scripting, Systemd, Cron, Shell Automation |
| **DevOps & Containerization** | Docker, Docker Compose, Git, GitHub, CI/CD Workflows |
| **Cloud Infrastructure** | AWS (EC2, EBS, Security Groups, IAM, S3, CloudWatch Basics) |
| **Networking & Security** | TCP/IP, DNS, SSH Key Auth & Tunneling, HTTP/HTTPS, Firewalls (UFW) |
| **Programming & Tooling** | Python, JavaScript (Node.js, React), YAML, Shell Scripts |

---

## 🧪 Featured Hands-on Labs & Projects

> 🚧 I'm currently building out my hands-on projects and will add them here as they're completed. Check back soon!

---

## 🗺️ DevOps Learning Roadmap

1. **Linux Administration** *(Mastered Basics)* - CLI, file permissions, process inspection, systemd.
2. **Bash Scripting** *(Mastered Basics)* - Log parsing, conditionals, functions, task automation.
3. **Networking Fundamentals** *(Mastered Basics)* - TCP/IP, DNS, SSH tunneling, firewalls.
4. **Git & GitHub** *(Mastered Basics)* - Branching models, PR workflows, interactive rebase.
5. **Docker & Containerization** *(Current Focus)* - Multi-stage builds, Docker Compose, bridge networks.
6. **Cloud Infrastructure (AWS)** *(Upcoming Goal)* - EC2, EBS, Security Groups, IAM.
7. **CI/CD Automation** *(Upcoming Goal)* - GitHub Actions workflow pipelines.
8. **Kubernetes** *(Upcoming Goal)* - Pods, Deployments, Services, Minikube/K3s.
9. **Infrastructure as Code** *(Future Goal)* - Declarative IaC with Terraform & Ansible.

---

## 🖥️ About This Web Application

This repository houses the source code for my interactive DevOps portfolio web application. It features:
- **Interactive Developer Terminal**: Command-line simulation (`docker ps`, `kubectl get pods`, `git status`, `systemctl status nginx`, `aws s3 ls`, `uname -a`, etc.)
- **Dark Mode DevOps Design**: Terminal-inspired aesthetic built with React, Vite, and Tailwind CSS.
- **GitHub Activity Feed**: Real-time project commits and contribution stats showcase.
- **Automated Deployment**: GitHub Actions pipeline automatically builds and deploys updates to GitHub Pages on every push to `main`.


## 📫 Connect With Me

- **Email**: [shahabshaikh70@gmail.com](mailto:shahabshaikh70@gmail.com)
- **GitHub**: [@shahab1729](https://github.com/shahab1729)
- **LinkedIn**: [Shahab Shaikh](https://www.linkedin.com/in/shahab-shaikh)
